...
public class ProductSellerService : IProductSellerService
{
    ...
    public ProductSellerService(ICommonService commonService, IProductRepository productRepository, ISellerRepository sellerRepository, ICategoryRepository categoryRepository, AppDbContext context, FileHelper fileHelper)
    {
    ...
    public async Task<ApiResponse<PagedResult<SellerProductDto>>> GetAllProductsAsync(PagedRequest request)
    {
        ...
        var seller = await _sellerRepository
            .FindSingleByConditionAsync(s => s.UserId == userId);
    ...
    {
        var seller = await _sellerRepository.FindSingleByConditionAsync(
            s => s.UserId == _commonService.GetUserId() && !s.DeleteFlg);
    ...
    public async Task<ApiResponse<string>> DeleteProductAsync(Guid productId)
    {
        ...
        if (hasOrders != null) return ApiResponse<string>.Fail("Cannot delete product with existing orders.Please change the status to 'Discontinued'or'Deleted'", 400, ErrorCodes.Product.CannotDeleteHasOrders);
