...
export class DashboardComponent implements OnInit{
  ...
  chartOptions:ChartConfiguration<'line'>['options']={
    ...
    scales:{
      ...
      y1:{type:'linear',position:'right',title:{display:true,text:this.translate.instant('ui.revenue')},grid:{drawOnChartArea:false},beginAtZero:true,ticks:{callback:(value)=>Number(value).toLocaleString('vi-VN')+'đ'}}
    }
  }
  ...
  buildChartData(pts:DashboardChartPointDto[]){
    ...
    this.chartData.set({
      ...
      datasets:[
        {label:this.translate.instant('ui.revenue'),data:pts.map(x=>x.revenue),yAxisID:'y1',hidden:sel!=='All' && sel!=='Revenue',borderColor:'blue',backgroundColor:'blue'},
        {label:this.translate.instant('ui.orders'),data:pts.map(x=>x.orders),yAxisID:'y',hidden:sel!=='All' && sel!=='Orders',borderColor:'green',backgroundColor:'green'},    
        {label:this.translate.instant('ui.admin.usersMetric'),data:pts.map(x=>x.users),yAxisID:'y',hidden:sel!=='All' && sel!=='Users',borderColor:'orange',backgroundColor:'orange'},
        {label:this.translate.instant('ui.admin.sellersMetric'),data:pts.map(x=>x.sellers),yAxisID:'y',hidden:sel!=='All' && sel!=='Sellers',borderColor:'purple',backgroundColor:'purple'},
      ]
    })
  }
}
