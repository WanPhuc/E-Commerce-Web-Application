...
using AuraMart.Identity.Domain.Repositories;
    public class AuthService : IAuthService
    {
        ...
        public async Task<ApiResponse<AuthResponseDto>> SignInAsync(SignInDto dto)
        {
            ...
            // Bu?c 1: Th? verify b?ng BCrypt (M?i)
            try
            {
            ...
            {
                // N?u l?i (do format hash cu kh�ng ph?i BCrypt), ta m?c d?nh false d? xu?ng check ti?p
                isValid = false;
            }

            // Bu?c 2: N?u BCrypt sai, th? check theo ki?u cu (V� d? SHA256)
            if (!isValid)
            {
                // 2. Th? SHA256 d?ng Hex (chu?i 64 k� t? nhu a1b2c3...)
                if (PasswordHelper.VerifyOldHash_SHA256(dto.Password, passwordHash))
                {
                    isValid = true;
                }
                // 3. Th? SHA256 d?ng Base64 (chu?i c� k� t? d?c bi?t nhu / + = ? cu?i)
                else if (PasswordHelper.VerifyOldHash_SHA256_Base64(dto.Password, passwordHash))
                {
                    isValid = true;
                }

                // N?u d�ng b?ng m?t trong hai c�ch cu, h�y n�ng c?p l�n BCrypt
                if (isValid)
                {
        ...
        public async Task<ApiResponse<AuthResponseDto>> SignUpAsync(SignUpDto dto)
        {
            ...
            try
            {
                ...
                return ApiResponse<AuthResponseDto>.Success(response, "�ang k� v� dang nh?p th�nh c�ng!", 201, SuccessCodes.Auth.SignUpSuccess);
            }
            catch (Exception ex)
            {
